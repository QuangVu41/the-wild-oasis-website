...
function DateSelector({ settings, bookedDates, cabin }) {
  ...
  return (
    <div className='flex flex-col justify-between'>
      <DayPicker
        className='pt-8 md:pt-12 place-self-center'
        mode='range'
      ...
      <div className='flex items-center justify-between px-4 md:px-8 bg-accent-500 text-primary-800 h-[72px]'>
        <div className='flex items-baseline gap-6'>
          ...
          {numNights ? (
            <>
              ...
              <p>
                <span className='text-base md:text-lg font-bold uppercase'>Total</span>{' '}
                <span className='text-base md:text-2xl font-semibold'>${cabinPrice}</span>
              </p>
            </>
          ) : null}
        </div>
        ...
        {range.from || range.to ? (
          <button className='border border-primary-800 py-2 px-2 md:px-4 text-sm font-semibold' onClick={resetRange}>
            Clear
          </button>
        ) : null}
      </div>
